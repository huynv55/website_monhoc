...
if (!empty($check)){
    ...
    foreach($check as $checkstuff){
    	$a=mysql_query("UPDATE  `quanlymonhoc`.`diemdanh` SET  `random_dd` =  '0' WHERE CONVERT(  `diemdanh`.`MaLop` USING utf8 ) =  '".$_GET[ip_mh]."' AND CONVERT(  `diemdanh`.`Masinhvien` USING utf8 ) =  '".htmlentities($checkstuff)."' AND `diemdanh`.`date` =  '".$date."' LIMIT 1 ;");
