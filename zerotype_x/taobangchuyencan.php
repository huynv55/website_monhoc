...
// Add some data
$sql3 = "SELECT DISTINCT  date FROM  diemdanh WHERE MaLop = '$_GET[ip_mh]' ORDER BY  date DESC";
...
$objPHPExcel->setActiveSheetIndex(0)
            ...
            ->setCellValue('A6', 'số buổi điểm danh')
            ->setCellValue('B6', $total)
            ->setCellValue('B7', 'Bảng điểm chuyên cần');
...
$objPHPExcel->setActiveSheetIndex(0)
            ...
            ->setCellValue('F9', 'chuyên cần')
            ->setCellValue('G9', 'điểm chuyên cần');
...
$objPHPExcel->getActiveSheet()->setCellValue('F'.$count, $row2['chuyencan']);
$objPHPExcel->getActiveSheet()->setCellValue('G'.$count, '=F'.$count.'/'.$total.'*10');
...
// Rename worksheet
$objPHPExcel->getActiveSheet()->setTitle('chuyencan');
...
header('Content-Disposition: attachment;filename="chuyencan.xlsx"');
